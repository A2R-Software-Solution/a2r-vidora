...
export default function YouTubeUnavailable({ onDismiss }) {
  ...
  useEffect(() => { const key = e => { if (e.code === "Space" || e.code === "ArrowUp") { e.preventDefault(); flap(); } }; window.addEventListener("keydown", key); return () => window.removeEventListener("keydown", key); }, []);
  useEffect(() => { if (game.status !== "playing") return; const timer = setInterval(() => setGame(g => { const v = g.velocity + .28, bird = g.bird + v; let pipeX = g.pipeX - 1.75, gapY = g.gapY, score = g.score; if (pipeX < -18) { pipeX = 100; gapY = 25 + Math.random() * 48; score++; }
    ...
    return birdTop < 0 || birdBottom > 100 || hit ? { ...g, bird, velocity: v, status: "over" } : { ...g, bird, velocity: v, pipeX, gapY, score }; }), 32); return () => clearInterval(timer); }, [game.status]);
